...
// TODO: We might want to clarify the semantics of this struct for safety reasons.
// Both `alloc` and `value` may become logically uninitialized because we either manually drop them
// or move them. I'm pretty sure that this temporary invariant breaking is fine so long as we don't
// run the struct's destructor or misuse the struct but it's better to be safe than sorry.
